...
void createSimpleGroundMesh(
    ...
    for (int z = 0; z <= gridSize; ++z) {
        for (int x = 0; x <= gridSize; ++x) {
            ...
            // Create height variation to force multiple polygons
            if (x >= 3 && x <= 4 && z >= 3 && z <= 7) {
...
int main() {
    ...
    // Step 1: Create simple test mesh
    std::cout << "[Step 1] Creating test mesh..." << std::endl;
    ...
    if (graph.nodeCount() > 0) {
        ...
        // Export DIMACS format for ERCA* solver
        success = GraphConverter::exportToDIMACSFormat(
            graph,
            "navmesh_cost.gr",
            "navmesh_res1.gr",
            "navmesh_res2.gr"
        ...
        if (!success) {
            std::cerr << "Failed to export DIMACS format files!" << std::endl;
        }
    } else {
